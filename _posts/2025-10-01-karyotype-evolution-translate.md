...
**核型**（*Karyotype*）包含一个物种的染色体数目，以及每条染色体的特征。核型演化反映了从祖先基因组到现代基因组的染色体变化，这些信息对重建祖先核型、追溯现代物种如何逐步演化至关重要。
...
> 简单说就是，从没有发生染色体变异的染色体出发，一步步地补全基因和染色体，组成祖先核型；再用这个祖先核型向其他物种比对。被选择的初始染色体全段最好与其他的染色体都有良好的共线性关系。
...
ACK 的所有原染色体均与每个现存基因组进行比较，并确定了这些原染色体组成的核型特征。根据 ACK 原染色体与现存物种染色体之间的融合和断裂位置，可推断核型变化及系统发育关系（图2c）。
...
所选物种对于核型重建至关重要。部分物种具有稳定的染色体结构，且大多数染色体为原染色体。例如，中华黄杨（*Buxus sinica*）拥有14条染色体，并经历了独立的全基因组复制事件，其染色体在一次全基因组复制后直接从 AEK 保留而来。我们绘制了 AEK 与中华黄杨的同源点图，通过水青树推断的 AEK 与中华黄杨的结果一致，表明核型重建结果具有唯一性（图10）。
...
**图13**    *比较连香树、欧亚葡萄与 AEK 的共线性关系点图，以选择合适的染色体重组 ACEK。图中的红色对号与绿色叉号表示后代的这条染色体是否与 AEK 的完全匹配*
...
使用的是水青树的参考基因组GCA_015143295.1](https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_015143295.1/)。自共线性结果看上去还不错，甚至优于作者的（图20）。
...
由于找不到连香树的基因组，我选择只使用葡萄（*V. vinifera*）的参考基因组[GCF_030704535.1](https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_030704535.1/)。先对葡萄和水青树做 blast、做共线性，再将原本水青树到 AEK 的映射延伸到葡萄上。可以看到，结果出现了细微偏差：葡萄的 Chr1 和 Chr 16 各自多出了一个条带；Chr 14 融合断点处的倒位也不见了（图21）。
...
遗憾的是，NCBI 数据库的圆叶葡萄参考基因组没有注释。于是我们选择了 2023 年 Matthew Huff 等人发表的基因组 [GCA_030254885.1](https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_030254885.1/)，该基因组基于圆叶葡萄 “Carlos” 品种，质量也算不错，就是 Contig N50 差了些（完整性 97.7%，Contig N50 11.3 Mb，Scaffold N50 20.8 Mb，测序深度 281x）[^9]。
...
将这些信息写到 map 上，再走一下 `-km`  `-ak` 和 `-akr`，就成功了。

具体方法如下：

1. 将这个 map 映射到圆叶葡萄基因组（`-km`），得到圆叶葡萄到祖先的完整 map；
2. 使用 `-km` 转换为祖先染色体核型；
3. 再使用 `-akr` 补充葡萄的基因信息。

> 不过需要注意，如果直接使用只有 6 列的 gff，它就会报错：
>
> ```
...
> ```
>
> 原本是用来检查 `.gff`（`df`）的第 6 列是否在 `.faa`（`pep`）的 keys 里。原因在于 `-ak` 和 `-akr` 接受的是 7 列（带旧名字的）GFF，而不是最原始的 6 列 GFF。该解决方法不会对后续结果造成影响。
>
...
[^5]: WGDI 的参数众多，需要的文件复杂，具体参数与文件准备可参考[该文章](https://hyli360.github.io/posts/WGDI/)。
