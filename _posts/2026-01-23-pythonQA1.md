...
Q：`@` 在Python上是装饰器标识符。迄今为止我已经遇到了三个装饰器：
...
A: 装饰器是 Python 非常棒的特性之一，个人感觉比那些“魔术方法”更“魔术”（因为“魔术方法”看上去既不直观也不魔术，但以后或许会说。或许吧……）
...
用函数伪装成属性，就是它最大的功用——这后面两个函数（方法）如同活板门，他们只能将值传出去，而无法接受一个值以进行改动。更妙的是，调用它们不需要带`()`，让它们看上去更像属性，而不是函数（方法）！
...
它让函数具备自动缓存的能力，从而在下次获得同样输入时，通过查表直接返回结果。令人欣慰的是，它的时间复杂度同样是线性阶 $O(N)$。
