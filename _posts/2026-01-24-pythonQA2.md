...
另外，这个方法的局限也很明显——你只能对第一个参数的类型进行分派，而无法实现多参数类型分派，这需要第三方库 `multipledispatch` 支持，而且后续要实现（参数1类型数量 * 参数2类型数量 * …… * 参数 N 类型数量）个分派分支，复杂到不如多定义几个函数。谁需要谁去用吧……
...
```py
@overload
def fasta_extractor(fasta: list[SeqRecord]) -> dict: ...
```
